                    ...
                    if instructions.lower().find("unassigned") < instructions.lower().find("cluster"):
                        ...
                        if dest_cluster:
                            for alert_id in alert_ids:
                                if alert_id in self.current_clusters["unassigned_alerts"]:
                                    print(f"DEBUG: Moving alert {alert_id} from unassigned to {cluster_id}")
                                    # Add to cluster
                                    if "alert_ids" not in dest_cluster:
                                        dest_cluster["alert_ids"] = []
                                    elif isinstance(dest_cluster["alert_ids"], str):
                                        # Handle case where alert_ids is stored as a string
                                        try:
                                            dest_cluster["alert_ids"] = json.loads(dest_cluster["alert_ids"])
                                        except:
                                            dest_cluster["alert_ids"] = [int(aid.strip()) for aid in dest_cluster["alert_ids"].replace('[', '').replace(']', '').split(',') if aid.strip()]
                        ...
                        if source_cluster and "alert_ids" in source_cluster:
                            # Handle case where alert_ids is stored as a string
                            if isinstance(source_cluster["alert_ids"], str):
                                try:
                                    source_cluster["alert_ids"] = json.loads(source_cluster["alert_ids"])
                                except:
                                    source_cluster["alert_ids"] = [int(aid.strip()) for aid in source_cluster["alert_ids"].replace('[', '').replace(']', '').split(',') if aid.strip()]
                            
                            for alert_id in alert_ids:
                                if alert_id in source_cluster["alert_ids"]:
                                    print(f"DEBUG: Moving alert {alert_id} from {cluster_id} to unassigned")
                                    # Remove from cluster
        ...
        if "merge" in instructions.lower() and "cluster" in instructions.lower():
            ...
            if len(cluster_matches) >= 2:
                ...
                if len(clusters_to_merge) == 2:
                    ...
                    # Handle case where alert_ids is stored as a string in either cluster
                    if isinstance(base_cluster.get("alert_ids", []), str):
                        try:
                            base_cluster["alert_ids"] = json.loads(base_cluster["alert_ids"])
                        except:
                            base_cluster["alert_ids"] = [int(aid.strip()) for aid in base_cluster["alert_ids"].replace('[', '').replace(']', '').split(',') if aid.strip()]
                    
                    if isinstance(merge_cluster.get("alert_ids", []), str):
                        try:
                            merge_cluster["alert_ids"] = json.loads(merge_cluster["alert_ids"])
                        except:
                            merge_cluster["alert_ids"] = [int(aid.strip()) for aid in merge_cluster["alert_ids"].replace('[', '').replace(']', '').split(',') if aid.strip()]
